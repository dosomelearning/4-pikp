...
echo "Scanning AQI categories from: ${RAW_CSV}"
...
with open(csv_path, newline="", encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        if progress_every > 0 and rows % progress_every == 0:
            print(
                f"[progress] rows={rows:,} distinct_aqi_categories={len(unique):,}",
                file=sys.stderr,
                flush=True,
            )

print(
    f"[summary] rows={rows:,} distinct_aqi_categories={len(unique):,}",
    file=sys.stderr,
    flush=True,
)
...
if [[ ! -s "${TMP_VALUES_FILE}" ]]; then
  echo "ERROR: no AQI categories extracted from ${RAW_CSV}"
  exit 1
fi

{
  cat <<'SQL'
CREATE TEMP TABLE stg_aqi_category (
    aqi_category_nk text PRIMARY KEY,
    aqi_category_name text NOT NULL
);
COPY stg_aqi_category (
    aqi_category_nk,
    aqi_category_name
...
INSERT INTO dw.dim_aqi_category (
    aqi_category_nk,
    aqi_category_name,
    valid_from,
    valid_to,
...
SELECT
    ...
    TRUE AS is_current
FROM stg_aqi_category s
LEFT JOIN dw.dim_aqi_category d
  ON d.aqi_category_nk = s.aqi_category_nk
 AND d.is_current = TRUE
WHERE d.aqi_category_nk IS NULL;

-- Ensure explicit unknown member exists for fact rows with missing category.
INSERT INTO dw.dim_aqi_category (
    aqi_category_nk,
    aqi_category_name,
    valid_from,
...
WHERE NOT EXISTS (
    SELECT 1
    FROM dw.dim_aqi_category d
    WHERE d.aqi_category_nk = 'unknown'
      AND d.is_current = TRUE
);

DROP TABLE stg_aqi_category;
SQL
} | docker compose --env-file "${ENV_FILE}" -f "${COMPOSE_FILE}" exec -T postgres \
  bash -lc "PGPASSWORD='${POSTGRES_PASSWORD}' psql -U '${POSTGRES_USER}' -d '${POSTGRES_DB}' -v ON_ERROR_STOP=1"

echo "Done. Current AQI-category cardinality:"
docker compose --env-file "${ENV_FILE}" -f "${COMPOSE_FILE}" exec -T postgres \
  bash -lc "PGPASSWORD='${POSTGRES_PASSWORD}' psql -U '${POSTGRES_USER}' -d '${POSTGRES_DB}' -c 'SELECT COUNT(*) AS dim_aqi_category_rows, COUNT(*) FILTER (WHERE is_current) AS current_rows FROM dw.dim_aqi_category;'"
