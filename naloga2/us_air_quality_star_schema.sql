...
-- Reuses shared dimensions:
--   - dw.dim_time (air daily rows map to HH=00 day anchor)
--   - dw.dim_location (air facts use county-level rows)
...
-- Grain: one row per county per day from daily_aqi_by_county
CREATE TABLE IF NOT EXISTS dw.fact_air_quality_daily (
    -- Source natural grain key
