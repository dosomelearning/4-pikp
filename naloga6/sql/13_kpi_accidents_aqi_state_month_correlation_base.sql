-- Dashboard 4: Base accident and AQI measures for relationship analysis.
-- Grain: one row per state and month where both accident and AQI data exist.
-- Intended Superset charts: scatter plots and mixed time-series comparison.
-- Interpretation note: this supports association analysis, not causal claims.
...
WITH accident_monthly AS (
    SELECT
        ...
        ROUND(PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY f.accident_duration_minutes)::numeric, 2) AS median_duration_minutes,
        ROUND(PERCENTILE_CONT(0.90) WITHIN GROUP (ORDER BY f.accident_duration_minutes)::numeric, 2) AS p90_duration_minutes
